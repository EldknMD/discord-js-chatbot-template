...
- [ ] A text editor installed on your device. Windows Notepad is enough. 
- [ ] [Node.js library](https://nodejs.org/en/download/current/) installed on your PC. You can see that version you need (32 or 64-bit) searching `system information panel` on your system. Panel picture in spanish bellow. I'll start the install wizard, as any other program.
...
As you click, a confirm prompt will pop out. **Bots can't be destroyed** in Discord API.
...
Copy the token using the `copy` blue button. and put it in a safe place. **If you suspect someone could be using your token, regenerate it immediatly**
...
>*Anybody who has never made a mistake, has never tried something new.* Albert Einstein
...
- *Option 1*: Download the repository directly from GitHub, on the `clone or download` label, then download as a zip (or Git client, if you got it in your PC). The same link is [here](https://github.com/EldknMD/discord-js-chatbot-template/archive/master.zip). I recommend this option if you're not familiar with the following option.
...
1. On `config.json`:
   ...
   - *Optional* **Your bot prefix**: A prefix is the "warning" message the bot is looking for start a script. Is a "listen up the following" message. The default prefix I set is `>`. You can change it to whatever your want. Even words or numbers. In my opinion, the simpler, the better: single character, easy to remember and type, but not easy enough to type it by mistake (`.` or `,` are bad options because of their position in a QWERTY keyboard and due to their commonness in english, spanish and other romanic languages). 
   ...
   - *Optional* **Your Discord user ID**: In order to get a quick and error log, the bot will dm you the cause of the error. This is specially recommended if you're planning build more code upon the one i put here.
...
3. A big blue screen (unless you changed that before) should appear now. Don't be afraid: this blue screen is what we want. 
